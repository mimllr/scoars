...
export async function GET() {
  ...
  if (!event) {
    return new ImageResponse(
      <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:'#111113' }}>
        <div style={{ display:'flex', color:'rgba(255,255,255,0.4)', fontSize:40, fontWeight:700, letterSpacing:4, textTransform:'uppercase' }}>No games today</div>
      </div>,
  ...
  const TeamCol = ({ abbr, logo, score, color, winning }) => (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center' }}>
      <div style={{ display:'flex', width:6, height:120, borderRadius:3, background: winning ? color : 'rgba(255,255,255,0.1)', marginBottom:32 }} />
      <img src={logo} width={148} height={148} style={{ objectFit:'contain' }} />
      <div style={{ display:'flex', marginTop:28, fontSize:54, fontWeight:800, letterSpacing:3, color: winning ? '#fff' : dim, textTransform:'uppercase' }}>{abbr}</div>
      {score != null && (
        <div style={{ display:'flex', marginTop:12, fontSize:100, fontWeight:800, lineHeight:1, color: winning ? '#fff' : dim }}>{score}</div>
      )}
    </div>
  );

  return new ImageResponse(
    <div style={{ width:'100%', height:'100%', display:'flex', flexDirection:'column', background:'#111113', padding:'52px 72px' }}>
      {/* top bar */}
      <div style={{ display:'flex', flexDirection:'row', alignItems:'center', marginBottom:40 }}>
        <div style={{ display:'flex', fontSize:20, fontWeight:800, letterSpacing:5, color:'rgba(255,255,255,0.25)', textTransform:'uppercase' }}>SCOARS</div>
        <div style={{ flex:1 }} />
        {isLive ? (
          <div style={{ display:'flex', flexDirection:'row', alignItems:'center', gap:10, background:'rgba(232,70,59,0.15)', border:'1px solid rgba(232,70,59,0.4)', borderRadius:8, padding:'6px 20px', marginRight:20 }}>
            <div style={{ display:'flex', width:9, height:9, borderRadius:'50%', background:'#E8463B' }} />
            <div style={{ display:'flex', fontSize:16, fontWeight:800, letterSpacing:3, color:'#E8463B', textTransform:'uppercase' }}>Live</div>
          </div>
        ) : null}
        <div style={{ display:'flex', fontSize:20, fontWeight:700, letterSpacing:2, color: isLive ? '#E8463B' : 'rgba(255,255,255,0.45)', textTransform:'uppercase' }}>{statusText}</div>
      </div>

      {/* teams row */}
      <div style={{ flex:1, display:'flex', flexDirection:'row', alignItems:'center' }}>
        <TeamCol abbr={awayAbbr} logo={awayLogo} score={awayScore} color={awayColor} winning={awayWinning} />
        <div style={{ display:'flex', width:2, height:200, background:'rgba(255,255,255,0.07)', margin:'0 48px' }} />
        <TeamCol abbr={homeAbbr} logo={homeLogo} score={homeScore} color={homeColor} winning={homeWinning} />
      </div>

      {/* venue */}
      <div style={{ display:'flex', justifyContent:'center', marginTop:32, fontSize:16, fontWeight:500, letterSpacing:2, color:'rgba(255,255,255,0.2)', textTransform:'uppercase' }}>
        {comp.venue?.fullName || ''}
      </div>
    </div>,
    { width: 1200, height: 630 }
